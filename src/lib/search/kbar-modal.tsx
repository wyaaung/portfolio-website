...
const KBarModal = ({ actions, isLoading }: { actions: Action[]; isLoading: boolean }) => {
        ...
        <KBarResults
          ...
          onRender={({ item, active }) => (
            <div>
              {typeof item === 'string' ? (
                <div className="pt-3">
                  <div className="block border-t border-gray-100 px-4 pb-2 pt-6 text-xs font-semibold uppercase text-cyan-600 dark:border-gray-800">
                    {item}
                  </div>
                </div>
              ) : (
  ...
  return (
    <KBarPortal>
      <KBarPositioner className="bg-gray-300/50 p-4 backdrop-blur backdrop-filter dark:bg-black/50 z-30">
        <KBarAnimator className="w-96 max-w-xl">
