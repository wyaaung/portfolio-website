...
const KBarModal = ({ actions, isLoading }: { actions: Action[]; isLoading: boolean }) => {
				...
				<KBarResults
					...
					onRender={({ item, active }) => (
						<div>
							{typeof item === 'string' ? (
								<div className='pt-3'>
									<div className='block border-gray-100 border-t px-4 pt-6 pb-2 font-semibold text-cyan-600 text-xs uppercase dark:border-gray-800'>
										{item}
									</div>
								</div>
							) : (
	...
	return (
		<KBarPortal>
			<KBarPositioner className='z-30 bg-gray-300/50 p-4 backdrop-blur backdrop-filter dark:bg-black/50'>
